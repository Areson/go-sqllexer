...
here */`,
		...
		{
			name:  "Identifier with partial backtick quoting",
			input: "SELECT test FROM `foo`.bar WHERE id = 1",
			expected: []TokenSpec{
				{COMMAND, "SELECT"},
				{SPACE, " "},
				{IDENT, "test"},
		...
		{
			name:  "Identifier with spaces around period",
			input: `SELECT test FROM foo . bar WHERE id = 1`,
			expected: []TokenSpec{
				{COMMAND, "SELECT"},
				{SPACE, " "},
		...
		{
			name:  "Identifier with partial backtick quoting with spaces",
			input: "SELECT test FROM `foo` . bar WHERE id = 1",
			expected: []TokenSpec{
				{COMMAND, "SELECT"},
				{SPACE, " "},
		...
		{
			name:  "Identifier with backtick quoting with spaces",
			input: "SELECT test FROM `foo` . `bar` WHERE id = 1",
			expected: []TokenSpec{
				{COMMAND, "SELECT"},
				{SPACE, " "},
